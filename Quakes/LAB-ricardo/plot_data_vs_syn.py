...
#		print(motion + ' is not a valid type of motion')
	nstart = '/Users/vidale/Documents/PyCode/LAB/ricardo/Mseed'
	if rotated:
		fname_dat       = nstart + '_data/rot/'            + comp_pre + event_no + '.mseed'
		fname_synH      = nstart + '_syn/rot/cvmhy/'       + comp_pre + event_no + '_cvmhy.mseed'
		fname_synS      = nstart + '_syn/rot/cvms426-223/' + comp_pre + event_no + '_cvms426-223.mseed'
		fname_synH_simp = nstart + '_syn/rot/cvmhn/'       + comp_pre + event_no + '_cvmhn.mseed'
		fname_synS_simp = nstart + '_syn/rot/cvms400-100/' + comp_pre + event_no + '_cvms400-100.mseed'
	else:
		fname_dat       = nstart + '_data/'+ motion + '/'             + comp_pre + event_no + '.mseed'
		fname_synH      = nstart + '_syn/' + motion + '/cvmhy/'       + comp_pre + event_no + '_cvmhy.mseed'
		fname_synS      = nstart + '_syn/' + motion + '/cvms426-223/' + comp_pre + event_no + '_cvms426-223.mseed'
		fname_synH_simp = nstart + '_syn/' + motion + '/cvmhn/'       + comp_pre + event_no + '_cvmhn.mseed'
		fname_synS_simp = nstart + '_syn/' + motion + '/cvms400-100/' + comp_pre + event_no + '_cvms400-100.mseed'
	...
	print('st_names len is ' + str(len(st_name)) + ' st_synH len is ' + str(len(st_synH)))
			...
			for ii in range(len(st_name)):  # find matching entry in station roster
				if tr.stats.network == st_netw[ii] and tr.stats.station == st_name[ii]: # find station in inventory
					if (st_dist[ii] < max_dist) and (st_dist[ii] > min_dist): # exclude stations too close or too far
						...
						for iii in range(len(st_synH)):  # find matching entry in station roster
							if tr.stats.network == st_synH[iii].stats.network and tr.stats.station == st_synH[iii].stats.station: # find same station in syn
								duration = 1
								if com_time: # trim data and syn traces to common time interval
									...
									if st_synH[iii].stats.starttime > s_t:
										s_t = st_synH[iii].stats.starttime
									if st_synH[iii].stats.endtime   < e_t:
										e_t = st_synH[iii].stats.endtime
									duration = e_t - s_t
									if duration > 0: # measure whether end_time is greater than start_time
										tr.trim(starttime=s_t,endtime = e_t)
										st_synH[iii].trim(starttime=s_t,endtime = e_t)
										st_synS[iii].trim(starttime=s_t,endtime = e_t)
										st_synH_simp[iii].trim(starttime=s_t,endtime = e_t)
										st_synS_simp[iii].trim(starttime=s_t,endtime = e_t)
				#					print(tr.stats.station + ' ' + str(dist1) + ' ' + str(dist2) + ' ' + str(dist3))
								if duration > 0 and (basin == False or (dist1 < basin_width) or (dist2 < basin_width) or (dist3 < basin_width)):  # keep stations only within X km of basin axis
				#						print('selected: ' + tr.stats.station)
									tr.stats.distance = st_dist[ii] # add distance to trace metadata
									st_synH[iii].stats.distance      = st_dist[ii]
									st_synS[iii].stats.distance      = st_dist[ii]
									st_synH_simp[iii].stats.distance = st_dist[ii]
									st_synS_simp[iii].stats.distance = st_dist[ii]
									st_dat_select += tr
									st_synH_select      += st_synH[iii]
									st_synS_select      += st_synS[iii]
									st_synH_simp_select += st_synH_simp[iii]
									st_synS_simp_select += st_synS_simp[iii]
...
#	plt.title(fname1[8:18] + ' vs ' + fname2[8:18])
	if rotated:
		if compo == 'Z':
			...
			plt.title(date_label + ' Radial velocity component: data vs synthetics')
		elif compo == 'N':
			plt.title(date_label + ' Transverse velocity component: data vs synthetics')
	else:
		plt.title(date_label + ' ' + compo + ' ' + motion + ' component: data vs synthetics')
	plt.show()
