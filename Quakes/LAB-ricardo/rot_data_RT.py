#!/usr/bin/env python
# program to rotate data to radial and transverse for LA basin motions
# John Vidale 7/2019

def rot_data_RT(event_no, start_buff, end_buff, taper, taper_frac, norm_each,
			plot_scale_fac, filt, freq_min, freq_max, min_dist, max_dist,
			 motion, basin_width, basin):
...
#%%  reject data on bad trace list, either individual components or A for all components
	...
	for tr in st_datZ: # examine traces one by one
		do_write = 1
		for ii in range(len(badt_event)):
#			if len(tr.data) == 0: # this will be a problem if one of N or E has data length of 0, below at max
#				do_write = 0
			if event_no == badt_event[ii] and tr.stats.station == badt_station[ii]: # find station in inventory
		...
		if do_write == 1:
			st_datZ_good += tr
	print('After rejecting labeled bad traces ones, E, N, Z data has '
	   + str(len(st_datE_good)) + ' ' + str(len(st_datN_good)) + ' ' + str(len(st_datZ_good))+ ' traces')

#%% trim data and syn traces to common time interval
	for ii in range(len(st_datE_good)):  # find matching entry in station roster
		s_t = st_datE_good[ii].stats.starttime
		e_t = st_datE_good[ii].stats.endtime
		# all syns have common time interval
		if st_datN_good[ii].stats.starttime > s_t:
			s_t = st_datN_good[ii].stats.starttime
		if st_datN_good[ii].stats.endtime   < e_t:
			e_t = st_datN_good[ii].stats.endtime
		duration = e_t - s_t
		if duration > 0: # measure whether end_time is greater than start_time
			tr.trim(starttime=s_t,endtime = e_t)
			st_datE_good[ii].trim(starttime=s_t,endtime = e_t)
			st_datE_good[ii].trim(starttime=s_t,endtime = e_t)
			st_datN_good[ii].trim(starttime=s_t,endtime = e_t)
			st_datN_good[ii].trim(starttime=s_t,endtime = e_t)
		else:
			print('Problem - some data have components that do not overlap in time, will not work')
# there's a problem here.  Trimming clips some off the vertical traces for no good reason
#		print('E length of of data ' + str(len(st_datZ_good[102].data)))

#%%  rotate horizontals, so N becomes radial, E becomes transverse
	trN     = Trace()
	trE     = Trace()
	ntrace = len(st_datE_good)
...
#		print('In rotate loop, ntrace is ' + str(ntrace))
		for ii in range(len(st_name)):  # find matching entry in station roster
			if (st_datE_good[iii].stats.network == st_netw[ii] and st_datE_good[iii].stats.station == st_name[ii]): # find station in inventory
					distance = gps2dist_azimuth( ev_lat, ev_lon, float(st_lat[ii]), float(st_lon[ii])) # station 14825
					baz = distance[2]  # back-azimuth
		ba = np.radians(baz)
		trN = st_datN_good[iii].copy()
		trE = st_datE_good[iii].copy()
#		print(st_datE_good[iii].stats.station + ' ' + str(baz))
		st_datN_good[iii].data = - (trE.data * np.sin(ba)) - (trN.data * np.cos(ba))
		st_datE_good[iii].data = - (trE.data * np.cos(ba)) + (trN.data * np.sin(ba))

	pathRT = '/Users/vidale/Documents/PyCode/LAB/ricardo/Mseed_data/rot/'
	st_datE_good.write(pathRT + 'vt' + '_' + event_no + '.mseed', format='MSEED')
	st_datN_good.write(pathRT + 'vr' + '_' + event_no + '.mseed', format='MSEED')
	st_datZ_good.write(pathRT + 'vz' + '_' + event_no + '.mseed', format='MSEED')

#%% select data by distance (and azimuth?)
...
#						print('selected: ' + tr.stats.station)
	...
	for tr in st_datZ_select:
		if len(tr.data) > 0:  # problem with losing Z data noted above during trim
			tr_max = max(abs(tr.data))*tr.stats.distance
			if tr_max > maxZ:
				maxZ = tr_max
...
#	plt.title(fname1[8:18] + ' vs ' + fname2[8:18])
	...
	plt.title(date_label + '  ' + event_no + ' 3C Vel  Transverse: black  Radial: green  Vertical: red')
	plt.show()
